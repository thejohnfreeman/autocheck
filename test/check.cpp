...
TEST(Check, Compiles) {
  ...
  //ac::check<std::string>(100, print_prop,
      //ac::make_arbitrary<std::string>(), ac::gtest_reporter());
  //ac::check<std::string>(100, print_prop,
  ...
  //auto cls =
    //ac::trivial([] (int, const std::vector<int>& xs) { return xs.empty(); },
        //ac::classify([] (int x, const std::vector<int>& xs) { return x < xs.front(); }, "at-head",
          //ac::classify([] (int x, const std::vector<int>& xs) { return xs.back() < x; }, "at-tail",
  ...
  ac::check<int, std::vector<int>>(100,
      ...
      ac::gtest_reporter(), std::move(cls));
}
